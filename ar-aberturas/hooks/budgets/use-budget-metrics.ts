...
import { normalize } from '@/helpers/budgets/normalize';
	...
	useEffect(() => {
		const fetchMetrics = async () => {
			try {
				...
				const { data: clientsWithBudget, error: clientsWithBudgetError } =
					await getClientsWithBudgetCount();
				...
				if (!soldError && soldBudgetsCount !== null) {
					setMetrics((prev) => ({
						...
						conversionRate:
							clientsCount > 0 ? Math.round((soldBudgetsCount / clientsCount) * 100) : 0,
					}));
				}
				...
				const { data: chosenAmounts, error: chosenAmountError } =
					await getChosenBudgetsTotalAmount();
				...
				const { data: budgetsByAmount, error: budgetsByAmountError } =
					await getBudgetsByAmountRange();
				...
				const { data: budgetsByAmountChosen, error: budgetsByAmountChosenError } =
					await getBudgetsByAmountRangeChosen();
				...
				const { data: budgetsByAmountSold, error: budgetsByAmountSoldError } =
					await getBudgetsByAmountRangeSold();
				...
				const { data: budgetsByAmountLost, error: budgetsByAmountLostError } =
					await getBudgetsByAmountRangeLost();
				...
				const { data: budgetsByLocation, error: budgetsByLocationError } =
					await getBudgetsByLocation();
				if (!budgetsByLocationError && budgetsByLocation) {
					const grouped = Object.values(
						budgetsByLocation.reduce(
							(acc, item) => {
				...
				const { data: clientsByContactMethod, error: clientsByContactMethodError } =
					await getClientsByContactMethod();
				...
				const { data: budgetsByMaterial, error: budgetsByMaterialError } =
					await getBudgetsByMaterial();
				...
				const { data: soldBudgetsByMaterial, error: soldBudgetsByMaterialError } =
					await getSoldBudgetsByMaterial();
				...
				const { data: soldBudgetsByMaterialByMonth, error: soldBudgetsByMaterialByMonthError } =
					await getSoldBudgetsByMaterialByMonth();
				...
				const { data: averageSaleDelayDays, error: avgDelayError } =
					await getAverageSaleDelayDays();
