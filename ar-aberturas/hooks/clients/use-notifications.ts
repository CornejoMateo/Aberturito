...
export function useNotifications() {
  ...
  const [selectedWork, setSelectedWork] =
    useState<WorkWithProgress | null>(null);
  ...
  const prepareNotification = async (
    ...
    try {
      ...
      const { data: client, error } =
        await getClientById(work.client_id);
  ...
  const openEmail = (work: WorkWithProgress) =>
    prepareNotification(work, 'email');

  const openWhatsApp = (work: WorkWithProgress) =>
    prepareNotification(work, 'whatsapp');
