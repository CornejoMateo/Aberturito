    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            ¿Estás seguro de que deseas {actionText} la cantidad de <span className="font-semibold">{itemName}</span>?
          </DialogDescription>
        </DialogHeader>
        <div className="py-4">
          <p className="text-center text-lg font-medium">
            {quantity} → <span className={action === 'increment' ? 'text-green-600' : 'text-red-600'}>{newQuantity}</span>
          </p>
        </div>
        <DialogFooter>
          <Button 
