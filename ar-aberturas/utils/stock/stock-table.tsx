...
export function StockTable({ filteredStock, onEdit, onDelete, onUpdateQuantity }: StockTableProps) {
  ...
  const [currentAction, setCurrentAction] = useState<{id: string, action: 'increment' | 'decrement', currentQty: number} | null>(null)
    ...
    <Card className="bg-card border-border overflow-hidden">
              ...
              filteredStock.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-secondary/50 transition-colors">
                    ...
                    <td className="px-2 py-2 whitespace-nowrap">
                      <div className="flex justify-center">
                        {(() => {
                          ...
                          return (
                            <Badge className={`gap-1 text-sm ${badgeColor}`}>
                              {label}
                            </Badge>
                          );
                        })()}
                    ...
                    <td className="px-2 py-2 whitespace-nowrap">
                      <p className="text-center text-sm text-muted-foreground">{item.site || 'N/A'}</p>
                    </td>
                    <td className="px-2 py-2 whitespace-nowrap">
      ...
      {currentAction && (
        <ConfirmUpdateDialog
          ...
          itemName={getItemName(filteredStock.find(item => item.id === currentAction.id) || {} as ProfileItemStock)}
