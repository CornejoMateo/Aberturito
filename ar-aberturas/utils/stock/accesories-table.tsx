					...
					<tbody className="divide-y divide-border">
						{filteredStock.length === 0 ? (
							...
							filteredStock.map((item) => {
								const total =
									((item as any)[keys.qtyForLump] ?? 0) * ((item as any)[keys.qtyLump] ?? 0) || 0;
								return (
									<tr key={(item as any).id} className="hover:bg-secondary/50 transition-colors">
										...
										<td className="px-2 py-2 whitespace-nowrap">
											<p className="text-center text-sm text-foreground">
												{(item as any)[keys.qtyForLump] ?? 0}
											</p>
										</td>
										<td className="px-2 py-2 whitespace-nowrap">
											<p className="text-center text-sm text-foreground">
												{(item as any)[keys.qtyLump] ?? 0}
											</p>
										</td>
										<td className="px-2 py-2 whitespace-nowrap">
												...
												<Button
													...
													onClick={() =>
														handleQuantityAction(
															(item as any).id,
															'decrement',
															(item as any)[keys.qty] ?? total
														)
													}
													disabled={
														(isUpdating && updatingId === (item as any).id) ||
														((item as any)[keys.qty] ?? total) <= 0
													}
												>
													<Minus className="h-3.5 w-3.5" />
												</Button>
												<div className="text-center min-w-[70px]">
													<p className="text-sm font-medium">{(item as any)[keys.qty] ?? total}</p>
										...
										<td className="px-2 py-2 whitespace-nowrap">
											<div className="flex justify-center">
												{(item as any)[keys.image] ? (
													<Button
														variant="outline"
														size="sm"
														onClick={() => setOpenImageUrl((item as any)[keys.image]!)}
													>
														Ver
													</Button>
												) : (
